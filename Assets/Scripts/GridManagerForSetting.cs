...
public class GridManagerForSetting : MonoBehaviour
{
    [Header("=== Grid ===")]
    public int rows = 10;
    public int cols = 10;
    ...
    void Start()
    {
        ...
        Debug.Log(FindObjectOfType<PlayerSettings>().PlayerRemainSteps);
        input.text = FindObjectOfType<PlayerSettings>().PlayerRemainSteps.ToString();

        gridCanvas = GameObject.Find("GridCanvas");

        // 从.csv文件读取网格数据
        LoadGridDataFromCSV(FindObjectOfType<PlayerSettings>().GetComponent<PlayerSettings>().csvData);
        title.text = FindObjectOfType<PlayerSettings>().GetComponent<PlayerSettings>().Title;
    ...
    public void StartGame()
    {
        ...
        FindObjectOfType<PlayerSettings>().PlayerRemainSteps = int.Parse(input.text);
        //FindObjectOfType<PlayerSettings>().csvData = csvFile.text;
    ...
    {
        FindObjectOfType<FileUploader>().OnButtonClick();
    }

    //public void LoadCSVString()
    //{
    //    FindObjectOfType<PlayerSettings>().csvData = csvFile.text;
    //    //Debug.Log("Back to settings!");
